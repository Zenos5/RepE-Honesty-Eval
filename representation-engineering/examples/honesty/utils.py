...
def honesty_function_dataset_truthqa(data_path: str, tokenizer: PreTrainedTokenizer, user_tag: str = "", assistant_tag: str = "", seed: int = 0) -> (list, list):
    """
    Processes data to create training and testing datasets based on honesty.
    ...
    # dataset_en = load_dataset(data_path, "en", split="validation")
    dataset_es = load_dataset(data_path, "es", split="validation")
    dataset = dataset_es
    # dataset = concatenate_datasets([dataset_en, dataset_es])

    # Load the data
    # df = pd.read_csv(data_path)
    df = dataset.to_pandas()
    # print(df)
    true_statements = df['best_answer'].values.tolist()
    # print(true_statements)
    # print(df['incorrect_answers'])
    # print([i[0] for i in df['incorrect_answers'].values.tolist()])
    false_statements = [i[0] for i in df['incorrect_answers'].values.tolist()]
...
def plot_detection_results(input_ids, rep_reader_scores_dict, THRESHOLD, start_answer_token=":"):
    ...
    plt.savefig("honesty_detection.png")
...
def plot_lat_scans(input_ids, rep_reader_scores_dict, layer_slice):
    for rep, scores in rep_reader_scores_dict.items():
        ...
        # print("scores:", scores)
        # print(input_ids)
        start_tok = input_ids.index('ĠA') #gpt
        # start_tok = input_ids.index('ĠASS') #qwen
        # start_tok = input_ids.index('▁A') #llama
        print(start_tok, np.array(scores).shape)
        print(np.array(scores))
        print(start_tok, layer_slice, np.array(scores).shape)
        # print(np.array(scores)[start_tok:start_tok+40,layer_slice])
        print(np.array(scores)[start_tok:])
        standardized_scores = np.array(scores)[start_tok:start_tok+40]
