...
from utils import honesty_function_dataset_truthqa, plot_lat_scans, plot_detection_results
...
# model_name_or_path = "TinyLlama/TinyLlama-1.1B-Chat-v1.0" # test
...
data_path = "HiTZ/truthfulqa-multi"
dataset = honesty_function_dataset_truthqa(data_path, tokenizer, user_tag, assistant_tag)
...
print(dataset['train']['labels'][0:4])

print(dataset['test']['data'][0:32])
...
plt.savefig('honesty_layers.png')
plt.show()
...
# dataset_en = load_dataset(data_path, "en", split="train")
dataset_es = load_dataset(data_path, "es", split="train")
dataset = dataset_es
# dataset = concatenate_datasets([dataset_en, dataset_es])

# Load the data
df = dataset.to_pandas()
test_input = df['question'].values.tolist()
